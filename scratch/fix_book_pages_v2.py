...
def extract_books_from_html(html_content):
    books = []
    # Find all book cards
    # Pattern: <div class="book-card">...</div>
    # Using a slightly more robust regex to handle potential variations
    ...
    for match in card_pattern.finditer(html_content):
        ...
        # Extract title from alt tag or title tag if exists
        ...
        # Extract author (if it was there, but most don't have it in the card yet)
        # We'll try to find it if we can, otherwise use empty
        author = ""
...
def fix_book_page(filename):
    ...
    original_title = title_match.group(1) if title_match else filename.replace("-", " ").title()
    ...
    for book in books:
        book_card = f"""
                    <div class="book-card">
                        <a href="{book['link']}" class="book-cover-link" target="_blank" rel="noopener noreferrer">
                            <img src="cover-placeholder.jpg" alt="{book['title']}" class="book-cover">
                        </a>
                        <div class="book-info">
                            <h3 class="book-title">{book['title']}</h3>
                            <div class="book-footer">
                                <span class="book-price">{book['price']}</span>
                                <a href="{book['link']}" class="btn-buy" target="_blank" rel="noopener noreferrer">BUY NOW</a>
                            </div>
                        </div>
                    </div>"""
        new_html += book_card
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print(f"Successfully updated {filename} with {len(books)} books.")

if __name__ == "__main__":
    for filename in BOOKS_FILES:
        fix_book_page(filename)
