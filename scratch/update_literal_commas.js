...
content = content.replace(/<div class="buffett-tab(?: active)?" id="([a-z]+)-tab-mobile" data-tab="\1">([\s\S]*?)<div class="mobile-books-see-more">/g, (match, tabName) => {
    ...
    // LITERAL COMMAS between items, wrapped in a span with a class if needed, but text is fine.
    const itemsHtml = terms[tabName].map((term, index) => {
        let aTag = `<a href="#" class="buffett-item-mobile news-headline">${term}</a>`;
        if (index < terms[tabName].length - 1) {
            aTag += `<span class="comma-text" style="color: #333; font-family: Georgia, serif; font-size: 14px; font-weight: 600;">, </span>`;
        }
        return aTag;
    }).join('\n                        ');
    return `<div class="buffett-tab${isActive}" id="${tabName}-tab-mobile" data-tab="${tabName}">
                    <div class="book-links-list comma-separated-list" id="${tabName}-buffett-slider">
                        ${itemsHtml}
                    </div>
...
content = content.replace(/<div class="tab-content(?: active)?" id="([a-z]+)-tab">([\s\S]*?)<div style="text-align: center; margin-top: 16px;">/g, (match, tabName) => {
    ...
    const isActive = tabName === 'philosophy' ? ' active' : '';
    const itemsHtml = terms[tabName].map((term, index) => {
        let aTag = `<a href="#" class="desktop-buffett-item news-headline">${term}</a>`;
        if (index < terms[tabName].length - 1) {
            aTag += `<span class="comma-text" style="color: #333; font-family: Georgia, serif; font-size: 14px; font-weight: 600;">, </span>`;
        }
        return aTag;
    ...
    return `<div class="tab-content${isActive}" id="${tabName}-tab">
                        <div class="buffett-topics comma-separated-list" style="display: block; padding: 0;">
                            ${itemsHtml}
                        </div>
...
console.log('Updated index.html with literal commas');

const cssPath = 'public/css/styles.css';
let css = fs.readFileSync(cssPath, 'utf8');
// Remove the old CSS pseudo element if it exists
css = css.replace(/\.comma-separated-list a:not\(:last-child\)::after {[\s\S]*?}/g, '');
css = css.replace(/\.comma-separated-list a::before {[\s\S]*?}/g, '');
css = css.replace(/\.comma-separated-list a:first-child::before {[\s\S]*?}/g, '');
